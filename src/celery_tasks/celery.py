...
celery_app = Celery(
    ...
    broker=container.configuration_registry().get('celery', {}).get('broker_url', 'redis://localhost:6379/0'),
    backend=container.configuration_registry().get('celery', {}).get('result_backend', 'django-db')
)
