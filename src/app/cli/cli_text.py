...
class LoadTextCommand(BaseCommand):
    ...
    def execute(self, input_dir):
        try:
            ...
            self.logger.error(
                f"Failed to load text files from directory {input_dir}: {e}"
            )
            raise
...
class ProcessTextCommand(BaseCommand):
    ...
    def __init__(
        self,
        registry=Provide[AppContainer.pipeline_component_registry],
        logger=Provide[AppContainer.struct_logger],
    ):
...
class SaveTextCommand(BaseCommand):
    ...
    def execute(self, format, output_dir):
        try:
            self.logger.info(
                f"Saving processed text files to {output_dir} in format: {format}"
            )
            self.text_saver.save(output_format=format, output_directory=output_dir)
            self.logger.info(
                f"Processed text files saved in {output_dir} as {format} files."
            )
        except Exception as e:
            self.logger.error(
                f"Failed to save processed text files in {output_dir}: {e}"
            )
            raise
