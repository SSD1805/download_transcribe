...
class TranscribeCommand(BaseCommand):
    ...
    def execute(self, input_directory, output_directory):
        try:
            ...
            self.logger.info(
                f"Transcription completed. Output saved in: {output_directory}"
            )
        except Exception as e:
            self.logger.error(
                f"Transcription failed for directory {input_directory}: {e}"
            )
            raise
...
class SaveTranscriptionCommand(BaseCommand):
    ...
    def execute(self, segments, audio_file, output_directory, format):
        try:
            self.logger.info(
                f"Saving transcription for audio file: {audio_file} to {output_directory} in format: {format}"
            )
            self.saver.save_transcription(segments, audio_file, format=format)
            self.logger.info(
                f"Transcription saved for {audio_file} in {output_directory} as {format}"
            )
        except Exception as e:
