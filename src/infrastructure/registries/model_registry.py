...
class ModelRegistry:
    """
    A registries for managing different models used in the system, such as transcribers and NLP models.
    ...
    def get_model(self, name: str) -> Any:
        ...
        if name not in self._registered_models:
            raise ValueError(f"Model '{name}' not found in registries.")
    ...
    def create_model(self, name: str, *args, **kwargs) -> Any:
        ...
        if name not in self._model_factories:
            raise ValueError(f"Model factory for '{name}' not found in registries.")
